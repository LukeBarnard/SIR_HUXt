...
def calibration_osse(params):
    ...
    output_dir = 'obs_lon_{:03d}_cme_{}_mp_v2'.format(lon_out, scenario)
...
if __name__ == "__main__":
    ...
    np.random.seed(19079502)
    #Generate processes equal to the number of cores
    params = itertools.product('slow_wide', lons)
    paramlist = list(params)
    with multiprocessing.Pool(processes=8) as pool
        res = pool.map(calibration_osse, paramlist)
    ...
    np.random.seed(19079502)
    #Generate processes equal to the number of cores
    params = itertools.product('fast_narrow', lons)
    paramlist = list(params)
    with multiprocessing.Pool(processes=8) as pool
        res = pool.map(calibration_osse, paramlist)
    ...
    np.random.seed(19079502)
    #Generate processes equal to the number of cores
    params = itertools.product('fast_wide', lons)
    paramlist = list(params)
    with multiprocessing.Pool(processes=8) as pool
        res = pool.map(calibration_osse, paramlist)
