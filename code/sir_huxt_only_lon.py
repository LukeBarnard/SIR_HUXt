...
def compute_resampling(parameter_array):
    ...
    # Force longitudes between -180:180
    lon[lon>180] -= 360

    # Convert speeds to z-scores
    lon_z, lon_avg, lon_std = zscore(lon)

    kde = KernelDensity(kernel='gaussian', bandwidth=0.25).fit(lon_z.reshape(-1,1), sample_weight=weights.ravel())
